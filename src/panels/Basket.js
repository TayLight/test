  ...
  return (
    <div className="Place">
      ...
      <div className="Place__choice">
        ...
        <div className="Place__choice-item">
          <h3>На месте</h3>
          <Checkbox checked={!selfService} onToggle={() => {setSelfService(!selfService);localStorage.setItem('selfService', ''+!selfService);}} />        </div>
      </div>
      <footer className="Place__footer">
          <Link to={`/order/${area.id}/${item.id}`} className="Place__order" onClick={(e) => {if(price <= 0 ){e.preventDefault(); alert('Корзина пуста, добавьте товар');}}}>
              Оплатить {price}
        </Link>
      </footer>
    </div>
